...
function App() {
  return (
    <div className="min-h-screen bg-[#fbfaf8] selection:bg-[#2C2B29] selection:text-white font-sans">
      ...
      <section className="bg-[#2C2B29] border-t border-b border-white/5 py-1 md:py-1.5 overflow-hidden relative z-20">
          ...
          {partnerLogos.map((logo, idx) => (
            <span key={`set1-${idx}`} className="shrink-0 flex items-center gap-12 md:gap-16">
              <img 
                src={logo.image} 
                alt={logo.name} 
                className="h-28 md:h-40 w-auto object-contain opacity-45 hover:opacity-85 transition-opacity duration-300 pointer-events-auto cursor-pointer"
